...
public class IndexadorArquivosLocais {
    private static String DIRETORIO_DOCUMENTOS = System.getProperty("user.home")
	    + "/Dropbox/treinamento/aulas-para-concursos";
    private static String DIRETORIO_INDICE = System.getProperty("user.home") + "/livro-lucene/aulas-concursos";
	...
	try {
	    ...
	    String dataIndexacaoFormatada = sdf.format(new Date(System.currentTimeMillis()));
	    //
	    doc.add(new TextField("conteudo", textoArquivo, Store.YES));
	    doc.add(new LongField("tamanho", Long.valueOf(arquivo.length()), Store.YES));
	    doc.add(new StringField("dataAtualizacao", dataArquivoFormatada, Store.YES));
	    doc.add(new StringField("dataIndexacao", dataIndexacaoFormatada, Store.YES));
