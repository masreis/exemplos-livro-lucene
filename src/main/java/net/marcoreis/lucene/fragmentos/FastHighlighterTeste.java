...
public class FastHighlighterTeste {
	...
	private static final Logger logger = LogManager.getLogger(FastHighlighterTeste.class);
	...
	public void highlight(Query query) {
		try {
			...
			for (ScoreDoc sd : docs.scoreDocs) {
				...
				String fragmentos = fhl.getBestFragment(fq, reader, sd.doc, CAMPO_COM_VETORES, 30);
	...
	public void highlightMultiPhraseQuery() {
		try {
			//
			Term[] termosAplicacaoExemplo = new Term[] { new Term("conteudoComVetores", "aplicação"),
